...
@NamedQueries({
    ...
    @NamedQuery(name = "Marca.findMaxId", query = "SELECT Max(m.id) FROM Marca m "),
    @NamedQuery(name = "Marca.findByDescMarca", query = "SELECT m FROM Marca m  WHERE m.nombre LIKE :descMarca order by m.nombre "),
